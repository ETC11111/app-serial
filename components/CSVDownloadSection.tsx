// components/CSVDownloadSection.tsx - 오프라인 데이터 내보내기 지원 강화
...
const CSVDownloadSection: React.FC<CSVDownloadSectionProps> = ({
        ...
        const getSensorLabel = (sensorType: number, valueIndex: number): { label: string; unit: string } => {
          switch (sensorType) {
            case 1: // 온습도센서
              ...
              break;
            case 2: // 조도센서
              return { label: '조도', unit: 'lx' };
            case 3: // ADS1115 수질센서 (pH, EC, 수온)
              if (valueIndex === 0) return { label: 'pH', unit: '' };
              if (valueIndex === 1) return { label: 'EC', unit: 'dS/m' };
              if (valueIndex === 2) return { label: '수온', unit: '°C' };
              break;
            case 4: // CO2센서
              ...
              break;
            case 5: // 온도센서
              return { label: '온도', unit: '°C' };
            case 19: // 토양센서 (pH, EC, 토양온도, 토양습도)
              if (valueIndex === 0) return { label: '토양pH', unit: '' };
              if (valueIndex === 1) return { label: '토양EC', unit: 'dS/m' };
              if (valueIndex === 2) return { label: '토양온도', unit: '°C' };
              if (valueIndex === 3) return { label: '토양습도', unit: '%' };
              break;
          }
  ...
  const getFriendlyName = (name: string): string => {
    const nameMap: { [key: string]: string } = {
      'SCD30_CH0': 'CO2센서', 'TSL2591_CH0': '조도센서', 'BH1750_CH0': '조도센서', 'SHT20_CH0': '온습도센서',
      'ADS1115_CH0': '수질센서', 'DS18B20_CH0': '수온센서', 'SOIL_SENSOR_CH0': '토양센서'
    };
  ...
  const extractSensorValue = (data: FlexibleSensorData, sensorName: string, valueIndex: number): number => {
    ...
    if (!sensor || !sensor.values || !sensor.values[valueIndex]) return 0;
    
    const value = sensor.values[valueIndex];
    if (typeof value !== 'number') return 0;
    
    // 센서 타입별 스케일링
    // 참고: 백엔드에서 이미 변환된 값이 전달되므로, 타입 3(ADS1115)과 타입 19(토양센서)는 추가 변환 불필요
    switch (sensor.type) {
      case 2: // 조도센서
        return value / 10;
      case 4: // CO2센서
        if (valueIndex === 0) return value / 100; // CO2
        return value;
      case 3: // ADS1115 (pH, EC, 수온) - 백엔드에서 이미 변환됨
      case 19: // 토양센서 (pH, EC, 토양온도, 토양습도) - 백엔드에서 이미 변환됨
      default:
        return value;
    }
  };
