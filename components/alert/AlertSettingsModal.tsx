...
const SensorStatusDisplay: React.FC<SensorStatusDisplayProps> = ({ sensorData }) => {
  ...
  return (
    <div className="bg-blue-50 rounded-lg p-4">
      <div className="flex items-center space-x-2 mb-3">
        <UIIcon name="chart" size="md" />
        <h4 className="font-semibold text-blue-800">현재 연결된 센서 (알림 설정 가능)</h4>
      </div>

      {alertableSensors.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {alertableSensors.map(sensor => {
            ...
            return (
              <div key={sensor.name} className="bg-white p-3 rounded border">
                  ...
                  {sensor.values.map((value, index) => {
                    ...
                    return (
                      <div key={index} className="text-xs text-gray-600">
                        {metadata.valueLabels[index] || `값 ${index + 1}`}:
                        <strong className="ml-1">{formatValue(value)}</strong>
                      </div>
                    );
                  })}
                </div>
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            >
              <option value="above">이상 (값이 임계값을 초과하면 알림)</option>
              <option value="below">이하 (값이 임계값 미만이면 알림)</option>
            </select>
          </div>
...
const AlertListItem: React.FC<AlertListItemProps> = ({ alert, latestSensorData, onToggle, onDelete, loading }) => {
  ...
  const getSensorLabel = (alert: Alert) => {
    ...
    return `레거시 센서: ${alert.sensor_type}`;
  };
          ...
          <div className="flex justify-between items-start">
            <div className="flex-1">
              ...
              <div className="flex flex-wrap items-center gap-4 mt-2 text-xs text-gray-600">
                <span>센서: {log.sensor_name || log.sensor_type}</span>
                ...
                {log.sensor_name && (
                  <span className="px-2 py-1 bg-blue-100 text-blue-800 rounded flex items-center space-x-1">
                    <UIIcon name="sensor" size="sm" />
                    <span>정밀센서 (값 {(log.value_index || 0) + 1})</span>
                  </span>
                )}
              </div>
            </div>
...
const AlternativeAlertsPage: React.FC = () => {
    ...
    latestSensorData.sensors.forEach((sensor) => {
      ...
      sensor.values.forEach((value, index) => {
        ...
        const option: SensorOption = {
          ...
          valueIndex: index,
          label: `${channel} - ${valueLabel}${unit ? ` (${unit})` : ''} (현재: ${formatValue(value)})`,
          unit: unit,
          currentValue: value
        };
  ...
  const handleSaveNewAlert = async () => {
    ...
    try {
      ...
      if (!selectedSensor) {
        showMessage('선택된 센서를 찾을 수 없습니다. 다시 선택해주세요.');
        return;
      }
      ...
      if (result.success) {
        ...
        showMessage('저장 실패: ' + result.message);
      }
    } catch (error) {
  ...
  const handleToggleAlert = async (alert: Alert) => {
    ...
    try {
      ...
      if (result.success) {
        await fetchAlerts();
        showMessage(`알림이 ${updatedAlert.is_active ? '활성화' : '비활성화'}되었습니다.`);
      } else {
        showMessage('토글 실패: ' + result.message);
      }
    } catch (error) {
      showMessage('토글 중 오류가 발생했습니다.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <Layout maxWidth="wide" padding="md" background="gray">
      ...
      <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
        ...
        <p className="text-gray-600">
          연결된 센서별 개별 값에 대한 세밀한 알림 설정을 관리하세요. 
        </p>
      </div>
            ...
            {sensorOptions.length === 0 && (
              <div className="p-8 text-center">
                ...
                <p className="text-gray-500 mb-4">
                  알림을 설정하려면 숫자 값을 전송하는 센서가 활성화되어야 합니다.<br />
                  (기상센서 및 질소/인/칼륨은 알림 설정에서 제외됩니다)
                </p>
                <button
                  onClick={() => {
                    fetchSensorData(selectedDeviceId);
